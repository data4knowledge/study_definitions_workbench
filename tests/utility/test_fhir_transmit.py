...
class TestFhirM11Transmit:
    ...
    async def test_fhir_m11_transmit(self, mock_session_local, mock_usdm, mock_ft, mock_user):
        mock_session_local.return_value = MagicMock()
        mock_usdm_instance = MagicMock()
        mock_usdm_instance.study_version.return_value = {"titles": {"Official Study Title": "Test"}}
...
class TestFhirSoaTransmit:
    ...
    @patch("app.utility.fhir_transmit.USDMJson")
    @patch("app.utility.fhir_transmit.SessionLocal")
    async def test_fhir_soa_transmit(self, mock_session_local, mock_usdm, mock_ft, mock_user):
        mock_session_local.return_value = MagicMock()
        mock_usdm_instance = MagicMock()
...
class TestFhirTransmit:
    ...
    async def test_success(self, mock_tx, mock_ep, mock_fhir, mock_cm, mock_user, mock_session):
        mock_tx.create.return_value = MagicMock()
        mock_ep.find.return_value = MagicMock(endpoint="https://fhir.test/api")
        mock_server = MagicMock()
        mock_server.put = AsyncMock(return_value={"success": True, "data": {"id": "bundle-123"}})
        mock_fhir.return_value = mock_server
        mock_cm.success = AsyncMock()

        details = {"titles": {"Official Study Title": "Study X"}}
        await fhir_transmit("M11", 1, 2, '{"data": "test"}', details, mock_user, mock_session)
    ...
    @patch("app.utility.fhir_transmit.Endpoint")
    @patch("app.utility.fhir_transmit.Transmission")
    async def test_failure(self, mock_tx, mock_ep, mock_fhir, mock_cm, mock_user, mock_session):
        mock_tx.create.return_value = MagicMock()
        mock_ep.find.return_value = MagicMock(endpoint="https://fhir.test/api")
        mock_server = MagicMock()
        mock_server.put = AsyncMock(return_value={"success": False, "message": "Bad request"})
        mock_fhir.return_value = mock_server
    ...
    @patch("app.utility.fhir_transmit.Endpoint")
    @patch("app.utility.fhir_transmit.Transmission")
    async def test_failure_long_message(self, mock_tx, mock_ep, mock_fhir, mock_cm, mock_user, mock_session):
        mock_tx.create.return_value = MagicMock()
        mock_ep.find.return_value = MagicMock(endpoint="https://fhir.test/api")
        long_msg = "x" * 200
        mock_server = MagicMock()
        mock_server.put = AsyncMock(return_value={"success": False, "message": long_msg})
        mock_fhir.return_value = mock_server
        mock_cm.error = AsyncMock()
