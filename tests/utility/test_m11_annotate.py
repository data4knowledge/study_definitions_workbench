...
class TestAnnotate:
    ...
    def test_injects_marker_for_matching_element(self):
        ...
        # Marker is inside the target div, so findings travel with
        # their element if the document is later restyled or moved.
    ...
    def test_unmatched_finding_goes_to_unplaced(self):
        ...
        assert "m11-doc-marker" not in result.html
    ...
    def test_marker_carries_native_tooltip_fallback(self):
        html = '<div data-m11-element="Full Title">A Trial</div>'
        finding = _finding(
            rule="M11_001", message="Required element missing."
        )
        result = annotate(html, [finding])
        # ``title=`` as a fallback when the side-panel JS fails —
        # native browser tooltip still surfaces rule id + message.
