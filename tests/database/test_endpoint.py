...
def test_create_new_endpoint(db):
    ...
    # Create a test user
    user = UserDB(identifier="user_endpoint_create", email="test_endpoint_create@example.com", display_name="Test User")
    db.add(user)
    db.commit()
    db.refresh(user)
...
def test_create_existing_endpoint(db):
    ...
    # Create a test user
    user1 = UserDB(identifier="user_endpoint_create1", email="test_endpoint_create1@example.com", display_name="Test User 1")
    db.add(user1)
    db.commit()
    db.refresh(user1)
    ...
    # Create another user
    user2 = UserDB(identifier="user_endpoint_create2", email="test_endpoint_create2@example.com", display_name="Test User 2")
    db.add(user2)
    db.commit()
    db.refresh(user2)
...
def test_create_invalid_endpoint(db):
    ...
    # Create a test user
    user = UserDB(identifier="user_endpoint_invalid", email="test_endpoint_invalid@example.com", display_name="Test User")
    db.add(user)
    db.commit()
    db.refresh(user)
...
def test_find_by_endpoint(db):
    ...
    # Create a test user
    user = UserDB(identifier="user_find_endpoint", email="test_find_endpoint@example.com", display_name="Test User")
    db.add(user)
    db.commit()
    db.refresh(user)
...
def test_find(db):
    ...
    # Create a test user
    user = UserDB(identifier="user_find_id", email="test_find_id@example.com", display_name="Test User")
    db.add(user)
    db.commit()
    db.refresh(user)
...
def test_debug(db):
    ...
    # Create a test user
    user = UserDB(identifier="user_debug", email="test_debug@example.com", display_name="Test User")
    db.add(user)
    db.commit()
    db.refresh(user)
...
def test_delete_endpoint_with_multiple_users(db):
    ...
    # Create test users
    user1 = UserDB(identifier="user_delete1", email="test_delete1@example.com", display_name="Test User 1")
    db.add(user1)
    db.commit()
    db.refresh(user1)
    
    user2 = UserDB(identifier="user_delete2", email="test_delete2@example.com", display_name="Test User 2")
    db.add(user2)
    db.commit()
    db.refresh(user2)
    ...
    db_endpoint = db.query(EndpointDB).filter(EndpointDB.endpoint == endpoint_url).first()
...
def test_delete_endpoint_with_single_user(db):
    ...
    # Create a test user
    user = UserDB(identifier="user_delete_single", email="test_delete_single@example.com", display_name="Test User")
    db.add(user)
    db.commit()
    db.refresh(user)
    ...
    user_endpoint = db.query(UserEndpointDB).filter(UserEndpointDB.endpoint_id == endpoint.id).all()
...
def test_delete_endpoint_with_no_users():
    ...
    mock_session.query.side_effect = lambda model: {
        EndpointDB: MagicMock(filter=lambda *args: MagicMock(first=lambda: mock_endpoint)),
        UserDB: MagicMock(filter=lambda *args: MagicMock(first=lambda: MagicMock(endpoints=MagicMock(remove=MagicMock())))),
        UserEndpointDB: MagicMock(filter=lambda *args: MagicMock(all=lambda: []))
    }[model]
    
    # Create an endpoint instance
    endpoint = Endpoint(id=1, name="Test Endpoint", endpoint="https://example.com/api", type="FHIR")
