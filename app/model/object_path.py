...
class ObjectPath:
    ...
    def _path(self, object_value: object, instruction: str) -> object:
        try:
            ...
            group_result = re.match(
                r"(?P<attribute>\w+)(\[@(?P<name>\w+)='(?P<value>\w+)'\])?(\[(?P<subpath_index>\S+)\])?",
                instruction,
            )
            if group_result:
                ...
                is_digits = (
                    True
                    if result["subpath_index"] and result["subpath_index"].isdigit()
                    else False
                )
                ...
                return (
                    object_value
                    if self._empty() or None
                    else self._path(object_value, self._pop())
                )
            else:
                application_logger.error(
