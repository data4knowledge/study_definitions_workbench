...
class FileImport(FileImportBase):
    ...
    def page(cls, page: int, size: int, user_id: int, session: Session) -> list[dict]:
        ...
        skip = (page - 1) * size
        count = (
            session.query(FileImportDB).filter(FileImportDB.user_id == user_id).count()
        )
        data = (
            session.query(FileImportDB)
            .filter(FileImportDB.user_id == user_id)
            .offset(skip)
            .limit(size)
            .all()
        )
    ...
    def delete(self, session: Session) -> int:
        try:
            record = (
                session.query(FileImportDB).filter(FileImportDB.id == self.id).first()
            )
