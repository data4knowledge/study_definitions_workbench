...
class Transmission(TransmissionBase):
    ...
    @classmethod
    def create(
        cls, version: int, study: str, status: str, user_id: int, session: Session
    ) -> "Transmission":
    ...
    def find(cls, id: int, session: Session) -> "Transmission":
        db_item = (
            session.query(TransmissionTable).filter(TransmissionTable.id == id).first()
        )
    ...
    def page(cls, page: int, size: int, user_id: int, session: Session) -> list[dict]:
        ...
        skip = (page - 1) * size
        count = (
            session.query(TransmissionTable)
            .filter(TransmissionTable.user_id == user_id)
            .count()
        )
        data = (
            session.query(TransmissionTable)
    ...
    def update_status(self, status: str, session: Session) -> "Transmission":
        db_item = (
            session.query(TransmissionTable)
            .filter(TransmissionTable.id == self.id)
            .first()
        )
