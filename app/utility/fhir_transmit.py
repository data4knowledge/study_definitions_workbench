...
def run_fhir_transmit(
    version_id: int, endpoint_id: int, version: str, user: User
) -> None:
    t = threading.Thread(
        target=asyncio.run,
        args=(fhir_transmit(version_id, endpoint_id, version, user),),
    )
    t.start()
...
async def fhir_transmit(version_id: int, endpoint_id: int, version: str, user: User):
    ...
    try:
        ...
        if response["success"]:
            message = (
                f"Succesful transmission of FHIR message: {response['data']['id']}"
            )
        else:
            error_text = (
        ...
        application_logger.exception(
            "Exception transmititng FHIR message from version '{version_id}' to endpoint: {endpoint_id}",
            e,
        )
        session.close()
        await connection_manager.error(
            f"Error encountered transmititng FHIR message from version '{version_id}' to endpoint: '{endpoint_id}'",
            str(user.id),
        )
