...
class Service:
    ...
    async def get(self, url):
        try:
            ...
            return (
                ...
                else self._failure("GET", response)
            )
    ...
    async def file_post(self, url, files, data={}):
        try:
            ...
            response = (
                await self._client.post(full_url, files=files, data=data)
                if data
                else await self._client.post(full_url, files=files)
            )
            return (
                self._success(response)
                if response.status_code in [200, 201]
                else self._failure("POST", response)
            )
        except httpx.HTTPError as e:
    ...
    async def post(self, url, data={}, timeout=None):
        try:
            ...
            response = await self._client.post(
                self._full_url(url), data=data, timeout=timeout, headers=headers
            )
            return (
                self._success(response)
                if response.status_code in [200, 201]
    ...
    async def delete(self, url, data={}):
        try:
            ...
            return (
                ...
                else self._failure("DELETE", response)
            )
        except httpx.HTTPError as e:
    ...
    def _exception(self, operation, e):
        message = (
            f"HTTPX '{operation}' operation raised exception {e.__class__.__name__}"
        )
