...
def save_excel(doc, full_path):
    ...
    for index, section in enumerate(merged):
        row = {
            ...
            "content": f"NCI_{index + 1}",
        }
        df_template.loc[len(df_template)] = row
    with pd.ExcelWriter(full_path) as writer:
        df_content.to_excel(
            writer, sheet_name="documentContent", header=True, index=False
        )
        df_template.to_excel(
            writer, sheet_name="sponsorTemplate", header=True, index=False
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog="USDM Content From '.docx'", description="")
